#!/usr/bin/env python3
"""SVD XT with reduced resolution to avoid OOM"""
...
# Reduced resolution for VRAM
WIDTH, HEIGHT = 768, 1344  # Instead of 1080x1920
    ...
    return {
        ...
        "3": {
            ...
            "inputs": {
                ...
                "width": WIDTH,
                "height": HEIGHT,
                "video_frames": 16,  # Reduced from 25
                "motion_bucket_id": 95,
                "fps": 8,
...
print("🎬 SVD XT (Reduced: 768x1344, 16 frames)")
print("=" * 50)

for i, img in enumerate(images, 1):
    print(f"Scene {i}: {img}")
    
    # Copy and resize for SVD
    src = COMFY_OUT / img
    dst = COMFY_INPUT / img
    shutil.copy(src, dst)
    
    # Run SVD with reduced settings
    wf = build_svd_workflow(img)
    try:
        result = queue_and_wait(wf)
        print(f"  ✅ Output: {result.name}\n")
    except TimeoutError:
        print(f"  ❌ Timeout — trying lower settings...")
        # Try with even lower
        wf["3"]["inputs"]["video_frames"] = 12
        wf["3"]["inputs"]["width"] = 640
        wf["3"]["inputs"]["height"] = 1120
        result = queue_and_wait(wf, timeout=180)
        print(f"  ✅ Low-res output: {result.name}\n")
    
    time.sleep(5)

print("✅ All SVD scenes completed!")
